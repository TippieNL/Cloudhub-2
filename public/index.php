...
*
* A favorite is the caller's own preference rather than a change to the file
* store, which is why a viewer may keep them, why read-only mode does not stop
* them, and why the route is on the write guard's exempt list -- CSRF is still
* checked for both writes.
*
...
* that for every visit. The clients read this list once and keep it in step
* with what they star.
*
...
// HEAD as for /api/files/download: asked first by the web client, which then
// lets the browser stream the version to disk.
if ($path === '/api/files/versions/download' && ($method === 'GET' || $method === 'HEAD')) api_try(function()use($fs, $method) {
    release_session_lock();
    $rel = $fs->relative($fs->existing((string)($_GET['path']??'')));
    $id = (string)($_GET['id']??'');
...
if ($path === '/api/files/upload' && $method === 'POST') api_try(function()use($fs, $config) {
    ...
    if (str_starts_with($path, '/webdav')) {
        ...
        \CloudHub\Services\handle_webdav($fs, $config, $path, $method, [
            ...
            'replacing' => function(string $full) use ($fs, $config): void {
    ...
    if (in_array($path, ['/', '/favorites', '/servers', '/browse', '/users', '/trash', '/storage', '/duplicates'], true)) {
