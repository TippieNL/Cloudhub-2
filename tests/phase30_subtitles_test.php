...
$models = $readKt('net/Models.kt');
$stores = $readKt('data/Stores.kt');

$checks['the app asks the same route the web player is rendered from'] =
    str_contains($api, 'suspend fun subtitles(path: String): List<SubtitleTrack>')
    && str_contains($api, 'fun subtitleUrl(path: String): HttpUrl')
    && str_contains($models, 'data class SubtitleTrack');
$checks['tracks are handed to Media3 as WebVTT, whatever the file is'] =
    str_contains($player, 'MediaItem.SubtitleConfiguration')
    && str_contains($player, 'setMimeType(MimeTypes.TEXT_VTT)');
...
// A film with no text tracks selects none; reading that as "off" would forget
// the language after every video that happens to have no subtitles.
