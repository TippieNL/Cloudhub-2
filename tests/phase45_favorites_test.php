 ...
 *
 * The repository runs against a real in-memory SQLite database and the trash
 * against a real directory tree; the route wiring is pinned against the source
 * and exercised over HTTP by tests/http/run.php, which needs MySQL.
 */
...
$checks['the Favorites page is served'] = str_contains($index, "['/', '/favorites', '/servers',");
...
$checks['the page has a Favorites screen in its navigation'] =
    str_contains($view, 'data-route="/favorites">Favorites</a>') && str_contains($view, 'id="favorites-page"');
$checks['a star is on every file card'] = str_contains($app, '${starButton(f.path)}');
$checks['starring goes through the API, both ways'] =
    str_contains($app, "api('/api/favorites', { method: on ? 'POST' : 'DELETE', body: { path } })");
$checks['the router shows the Favorites page'] = str_contains($app, "if (p === '/favorites') {");
$checks['cards are wired per container'] = str_contains($app, "root.querySelectorAll('[data-fav]')")
    && !str_contains($app, "document.querySelectorAll('[data-open]')");
$checks['signing out forgets the stars'] = str_contains($app, "S.favorites = new Set();\n    FAV.entries = [];");
$checks['the star survives the narrow list view'] =
    str_contains($css, '.file-list-view .actions button:not([data-menu]):not([data-fav]){display:none}');

// --- the Android client ------------------------------------------------------------
$kotlin = $root.'/android/app/src/main/java/nl/tippie/cloudhub';
$api = (string)file_get_contents($kotlin.'/net/CloudHubApi.kt');
$main = (string)file_get_contents($kotlin.'/MainActivity.kt');
$model = (string)file_get_contents($kotlin.'/ui/FilesViewModel.kt');
$sheet = (string)file_get_contents($kotlin.'/ui/Dialogs.kt');
$checks['the app reads, adds and removes favorites'] =
    str_contains($api, 'get("/api/favorites")')
    && str_contains($api, 'postJson("/api/favorites",')
    && str_contains($api, 'request("/api/favorites", "DELETE",');
$checks['the app has a Favorites screen'] =
    str_contains($main, 'data object Favorites : Screen') && str_contains($main, 'is Screen.Favorites -> FavoritesScreen(');
$checks['a new sign-in starts with no stars'] =
    str_contains($model, 'it.copy(favorites = emptySet(), favoriteEntries = emptyList())');
$checks['anything that can move a file reloads the stars'] =
    (bool)preg_match('/fun refresh\(\) \{\s*open\(_state\.value\.path\)\s*loadFavorites\(\)/', $model);
$checks['the star is offered whatever the role'] = (function () use ($sheet): bool {
    $star = strpos($sheet, '"Add to favorites"');
    $gate = strpos($sheet, 'if (canWrite) {');
    return $star !== false && $gate !== false && $star < $gate;
})();
